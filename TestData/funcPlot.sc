...
funcPlot(xmin, xmax) {
	...
	plot.Aspect(0);
	plot.Window(xmin - xmargin, ymin - ymargin, xmax + xmargin, ymax + ymargin);
	fx = xmin;
	fy = func(xmin);
	...
	for (x = roundTop(xmin); x <= xmax; x += xstep) {
		x = roundRound(x, 5);
		if (x < xmin) continue;
		plot.Line(x, ymin, x, ymax);
		plot.Text(x, x, ymin, fontsize, 0, 1, 0);
	}
	ystep = (ymax - ymin) / 4;
	ystep = roundTop(ystep);
	for (y = roundTop(ymin); y <= ymax; y +=ystep) {
		y = roundRound(y, 5);
		plot.Line(xmin, y, xmax, y);
		plot.Text(y, xmin, y,fontsize, 0, 2, 1);
	}
